...
const CurriculumOverview = () => {
  ...
  useEffect(() => {
    const fetchCurriculums = async () => {
      ...
      try {
        ...
        const curriculumData = res.data.success ? res.data.data : res.data;

        // Ensure curriculumData is an array
        if (Array.isArray(curriculumData)) {
          setCurriculums(curriculumData);
          if (curriculumData.length > 0) {
            setSelectedCurriculumId(curriculumData[0]._id);
          }
        } else {
          console.warn(
            "Expected array but got:",
            typeof curriculumData,
            curriculumData
          );
          setCurriculums([]);
        }
  ...
  const selectedCurriculum = Array.isArray(curriculums)
    ? curriculums.find((curr) => curr._id === selectedCurriculumId)
    : null;
    ...
    return (
      <section className="curriculum-sec">
        <div className="curriculum-heading">
          ...
          <p className="curriculum-sub-heading">Loading curriculum...</p>
        </div>
      </section>
    );
  }
    ...
    return (
      <section className="curriculum-sec">
        <div className="curriculum-heading">
          ...
          <p className="curriculum-sub-heading">{error}</p>
        </div>
      </section>
    );
  }
    ...
    return (
      <section className="curriculum-sec">
        <div className="curriculum-heading">
          ...
          <p className="curriculum-sub-heading">
            No curriculum data available for {program} program.
          </p>
        </div>
      </section>
    );
